...
Software engineer investigating warehouse AMR navigation failures daily.

## Goal

Read estimator covariance trajectories from first principles and diagnose EKF failures from logs and bags.

---

## Why This Track Exists

You already know the navigation estimator code deeply:
...
- The `isFinite()` check in `executePoseGoal()` triggers `ESTIMATED_STATE_NOT_FINITE`.
...
**Reading order:** 01 → 02 → 03 → 04 → 05. Each step depends on the previous one.
...
| -- | ---- | --------- | ----------------- |
| 01 | `01-dead-reckoning.md` | 3 hrs | Derive `(x', y', θ')` from encoder counts and understand AMR `predict()` noise terms. |
| 02 | `02-kalman-filter.md` | 4 hrs | KF → EKF equations and why covariance grows during motion and shrinks on measurement. |
| 03 | `03-measurement-models.md` | 3 hrs | Line-sensor constraints, Mahalanobis distance, and when AMR rejects a line-sensor update. |
| 04 | `04-imu-fusion.md` | 3 hrs | Gyro integration, bias estimation, and how the AMR theta update differs from XY updates. |
| 05 | `05-failure-modes.md` | 3 hrs | Recognize slip vs delocalize vs collision from covariance patterns in bags. |
...
After each file, answer these without opening any notes.

### After `01-dead-reckoning.md`

- [ ] A robot moves 0.1 m forward. Left wheel encoder reads 100 ticks, right reads 98. Wheel radius 0.05 m, ticks/rev = 1000, base = 0.3 m. Compute Δx, Δy, Δθ.
...
- [ ] Write the EKF prediction equations. What are `F`, `Q`, and `P`?
...
- [ ] The line-sensor reads a line at 5 cm left of expected. The covariance is large. Is the update accepted or rejected?
...
- [ ] If the IMU is missing, what happens to the robot theta update? Hint: check the line-sensor code path.
...
- [ ] The robot delocalized but slip detection never fired. What else can cause `ERROR_STATE_INVALID`?
...
**Connect:** The AMR prediction step is a simplified EKF. It does not use a Jacobian for covariance and relies on a fixed noise model instead. Understand why this is an approximation and when it breaks.

### Week 3: Measurement Update + Mahalanobis (3 hrs)

**Read:** `03-measurement-models.md`

**Do:** `exercises/03-ekf-unicycle.md` — measurement models, line constraints, and Mahalanobis gating drills.
...
**Do:** `exercises/04-log-diagnosis.md` — gyro bias, theta fusion, and IMU correction reasoning.

**Connect:** Look at `imuCallback()`. The gyro integration and bias correction will now make sense.

### Week 5: AMR Failure Modes + Bag Diagnosis (3 hrs)

**Read:** `05-failure-modes.md`

**Do:** `exercises/05-robot-specific.md` — walk failure scenarios and name the cause before opening the answer.

**Goal:** At the end of this week, every `ERROR_STATE_INVALID` ticket you open should have a 3-sentence hypothesis within 2 minutes of looking at the bag.
...
| `VelocityWindowTracker` | Velocity-window state tracking |
